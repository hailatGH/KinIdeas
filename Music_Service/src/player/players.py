...
def play_single_track(request):
    track_data = {}
    try:
        track_obj = MusicTrack.objects.all().values('id', 'track_name', 'track_file', 'artist_id', 'album_id', "genre_id")
        for i in range(len(track_obj)):
            ...
            artist_name = MusicArtist.objects.filter(id=track_obj[i]['artist_id']).values('artist_name')[0]['artist_name']
            album_id = track_obj[i]['album_id']
            album_title = MusicAlbum.objects.filter(id=track_obj[i]['album_id']).values('album_title')[0]['album_title']
            album_cover = MusicAlbum.objects.filter(id=track_obj[i]['album_id']).values('album_cover')[0]['album_cover']
            genre_id = track_obj[i]['genre_id']
            genre_title = MusicGenre.objects.filter(id=track_obj[i]['genre_id']).values('genre_title')[0]['genre_title']
            try:
                lyrics_id = MusicLyrics.objects.filter(track_id=track_id).values('id')[0]['id']
                lyrics_detail = MusicLyrics.objects.filter(track_id=track_id).values('lyrics_detail')[0]['lyrics_detail']
            except:
