...
def play_album_tracks(request):
    ...
    for i in range(len(track_obj)):
        ...
        lyrics_id = MusicLyrics.objects.filter(id=track_id).values('id')[0]['id']
        lyrics_detail = MusicLyrics.objects.filter(id=track_id).values('lyrics_detail')[0]['lyrics_detail']
